<template>
    ...
    <blood-sugar-result-male v-if="printIsTrue">
        ...
        <template #label> {{fastVal || ranVal || hsrVal}}</template> 
        <template #sDate> {{currentDateTime()}}</template>


        <!--pdf ends here]  -->
    </blood-sugar-result-male>
...
import BloodSugarResultMale from '../UI/BloodSugarResultMale.vue';
export default {
    data() {
        return{
            pharmName:'',
  ...
  components: { BaseCard, BloodSugarResultMale  },
